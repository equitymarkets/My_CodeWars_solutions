...
the program input, a string, possibly empty, that will be interpreted as an array of bytes using each character's ASCII code 
and will be consumed by the , instruction
... and will return ...
...
If you see test cases that require you to perform error-handling then please open an Issue in the Discourse for this Kata (don't forget to state 
which programming language you are attempting this Kata in).
*/
...
int recursive_loop(int i, )
  {
    int flag = i, ;
    if(code[i] == ']' && COUNT > 0) { i = flag-1; COUNT--; }
          else continue;
    return 0;
  }
...
std::string brainLuck(const std::string &code, const std::string &input) 
  {
    ...
    char long_array[5000] = {0};
    strcpy(long_array, input.c_str());
    char* ptr = long_array;
    for(int i = 0; i < code_length; i++)
      {
        int COUNT = 0, flag = 0;
        if(code[i] == '[') { flag = i; continue; } 
        
        if(code[i] == '>') { ++ptr; continue; }
        if(code[i] == '<') { --ptr; continue; }
        if(code[i] == '+') { ++*ptr; ++COUNT; continue; }
        if(code[i] == '-') { --*ptr; continue; }
        if(code[i] == '.') { putchar(*ptr); continue; }
        if(code[i] == ',') { *ptr = getchar(); continue; }
      }
  for(int i = 0; i < code_length; i++)
    {
      std::cout << long_array[i] << std::endl;
    }  
